...
return [
	'QueueScheduler' => [
		...
		// anchor the builder to the host app rather than the QueueScheduler plugin.
		...
		// After this many consecutive aborts (without an intervening success)
		// the row is disabled and a `QueueScheduler.Row.disabled` event is fired
		// so the host app can alert. 0 (default) = unlimited reruns, never
		// auto-disable. Requires the queue to record terminal "aborted" state.
